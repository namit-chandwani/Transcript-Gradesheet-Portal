...
def hodprojectapprove(request, project):
    ...
    if request.POST:
        ...
        print(comment)
        bitsid = project.bitsID#str(request.POST.getlist('bitsID')[0])#'2015A3PS0248G'
        toaddr = project.corrEmail#str(request.POST.getlist('corrEmail')[0])#"f20160052@goa.bits-pilani.ac.in"
        msg = MIMEMultipart() 
        msg['From'] = fromaddr 
        msg['To'] = toaddr 
        msg['Subject'] = 'Offcial Transcript'
...
def transcript2(request, bits_id):
    ...
    if (transcriptform.gradStatus == 'continuing'):
        transcriptform.amtcal = int(transcriptform.origTranscript)*200 + int(transcriptform.dupTranscripts)*100
    else:
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            for i in range(int(no_addr)):
                ...
                addrForm.transcript = Transcript.objects.get(form_no = transcriptform.form_no)
                addrForm.save()
...
def transcript(request):
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            transcriptform.postalAddressLocation = request.POST.get('postalAddressLocation')
...
def gradesheet(request):
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            gradesheetform.postalAddressLocation = request.POST.get('postalAddressLocation')
...
def gradesheet2(request, bits_id):
    ...
    if request.POST:
        form = GradesheetForm2(request.POST, extra = str(no_addr), sems = str(no_sems))
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            for form in formsDict[bits_id]:
                form.gradesheet = Gradesheet.objects.get(form_no = gradesheetform.form_no)
                form.save()
